...
These scripts capture SPEC CPU2017 reference workload commands from fake harness runs and replay them. A manifest captures two kinds of commands: benchmark invocations (`commands`) and the compare/specdiff invocations used to verify their output (`verify_commands`). It does not capture build commands, PGO training commands, or input-generation commands.

`generate_command_file.py` writes JSON manifests only. Shell command files are not generated by the current workflow.

The repository ships a prebuilt manifest, `commands.static.full.json`, which `verify_runs.py` reads by default. `generate_command_file.py` writes to `spec_run_commands.json` by default, and `run_spec_with_wrapper.py` reads `spec_integer_run_commands.json` by default; pass `--commands-file`/`--output` to override. Regenerate a manifest per install/config rather than reusing one across machines.
...
- `phase`: command phase. Entries in `commands` use `benchmark`; entries in `verify_commands` use `verify`.
...
`verify_runs.py` compares generated benchmark outputs without rebuilding or rerunning benchmarks:
...
- If the manifest has a non-empty `verify_commands` list, it replays each captured compare/specdiff command directly, in `( cd <run-directory> && ... )` form.
- Otherwise it falls back to finding each unique run directory and running that directory's `compare.cmd` through `specinvoke`.
...
`generate_command_file.py` regenerates the concrete command manifest for a specific SPEC config. It runs the selected suites through SPEC clobber, build, setup, and fake run phases, parses the fake benchmark invocations into `commands` and the fake compare invocations into `verify_commands`, and writes the same JSON schema consumed by `run_spec_with_wrapper.py` and `verify_runs.py`.
