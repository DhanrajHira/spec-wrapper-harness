...
```bash
./run_command_capture/generate_command_file.py \
  --config myconf.cfg \
  --build-ncpus 96 \
  --output run_command_capture/spec_run_commands.json
```
...
./run_command_capture/run_spec_with_wrapper.py \
  --install-root "$PWD" \
  --commands-file run_command_capture/spec_run_commands.json \
  --jobs -1 \
...
./run_command_capture/generate_command_file.py \
  --config myconf.cfg \
  --build-ncpus 96 \
  --output run_command_capture/spec_integer_run_commands.json
```
...
- `--log-dir PATH`: choose where generated fake-run logs are stored.
